  ...
  return (
    <section id="contact" className="relative py-16 sm:py-24 bg-[#171717] border-t border-[#2b2b2b]">
      <SectionQuickActions
        ...
        titleAr="التواصل والحجز (Contact)"
        badge={isAr ? 'بدء التعاون الإبداعي' : 'INITIATE COLLABORATION'}
        headerTitle={contact.ctaHeading}
        headerDescription={contact.ctaSubtitle}
        onUpdateHeader={(badge, title, description) => {
          updateContact({ ctaHeading: title, ctaSubtitle: description });
        }}
      />
          ...
          <div className="lg:col-span-5 space-y-6 sm:space-y-8">
            <div className="space-y-3">
              ...
              <h2 className="text-2xl sm:text-5xl font-black text-[#f1f2ed] tracking-tight uppercase font-quicksand leading-tight">
                <EditableText
                  value={contact.ctaHeading || (isAr ? 'لنصنع معاً عملاً بصرياً لا يُنسى' : "LET'S CREATE SOMETHING UNFORGETTABLE")}
                  onSave={(val) => updateContact({ ctaHeading: val })}
                  label={isAr ? 'عنوان التواصل' : 'Contact Heading'}
                />
              </h2>
              <div className="text-xs sm:text-base text-[#a8a6a1] leading-relaxed">
                <EditableText
                  value={contact.ctaSubtitle || (isAr 
                    ? 'هل لديك حملة إعلانية، فيلم وثائقي، أو مناسبة استثنائية ترغب في توثيقها بأعلى معايير السينما؟ تواصل معنا لنناقش رؤيتك.'
                    : "Have a commercial campaign, wedding, medical production, or creative concept? Share your vision and let's craft cinematic impact together.")}
                  onSave={(val) => updateContact({ ctaSubtitle: val })}
                  multiline
                  label={isAr ? 'الوصف الترحيبي' : 'Subtitle'}
                />
              </div>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3 sm:gap-4 p-3.5 sm:p-4 rounded-xl bg-[#1d1d1d] border border-[#2b2b2b] hover:border-[#941e33]/60 transition-colors group">
                <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-lg bg-[#232323] text-[#b8283f] group-hover:bg-[#941e33] group-hover:text-white flex items-center justify-center transition-colors flex-shrink-0">
              ...
              <div className="flex items-center gap-2.5">
                {contact.instagram && (
                  <a
                    ...
                    className="p-3 rounded-xl bg-[#1d1d1d] border border-[#2b2b2b] hover:border-[#941e33] text-[#a8a6a1] hover:text-white hover:bg-[#941e33] transition-all"
                    title="Instagram @mografiix"
                  >
            ...
            {contact.responseTimeNote && (
              <div className="flex items-center gap-2 text-xs text-[#a8a6a1] font-mono">
                <Clock className="w-3.5 h-3.5 text-[#941e33]" />
            ...
            <div className="p-5 sm:p-8 rounded-2xl bg-[#1d1d1d] border border-[#2b2b2b] shadow-2xl relative">
              {submitted ? (
                <div className="py-12 sm:py-16 text-center space-y-4 animate-in fade-in duration-300">
                  <div className="w-14 sm:w-16 h-14 sm:h-16 rounded-full bg-[#941e33]/20 border border-[#941e33] text-[#b8283f] flex items-center justify-center mx-auto">
                  ...
                  <p className="text-xs sm:text-sm text-[#a8a6a1] max-w-md mx-auto">
                    {isAr 
                      ? 'شكراً لتواصلك. سيقوم المخرج محمد بمراجعة تفاصيل المشروع والتواصل معك خلال 24 ساعة.'
                      : 'Thank you for reaching out. Mohammad will review your production request and respond within 24 business hours.'}
                  </p>
                  <button
                    onClick={() => {
                      setSubmitted(false);
                ...
                <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full px-4 py-2.5 rounded-xl bg-[#232323] border border-[#2b2b2b] focus:border-[#941e33] focus:outline-none text-xs sm:text-sm text-[#f1f2ed] resize-none"
                    />
                  </div>
                  ...
                  <button
                    ...
                    type="submit"
                    className="w-full py-3.5 sm:py-4 rounded-xl font-bold text-xs uppercase tracking-wider text-white bg-[#941e33] hover:bg-[#b8283f] transition-all duration-200 shadow-xl flex items-center justify-center gap-2 border border-[#b8283f]/40 cursor-pointer"
                  >
